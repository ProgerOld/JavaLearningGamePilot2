...
import com.example.javalearninggamepilot2.entity.Asteroid;
import com.example.javalearninggamepilot2.entity.Background;
import com.example.javalearninggamepilot2.entity.Bullet;
import com.example.javalearninggamepilot2.entity.Flight;
...
public class GameView extends SurfaceView implements Runnable{
    ...
    private void update() {
        ...
        int getY = flight.getY(); //Получить положение самолета
        //Log.i("FLY", String.valueOf(getY));

        if (flight.isGoingUp()) { // условие подъёма
            int setY = flight.getY() - (int)(30 * screenRatioY);
            flight.setY(setY);
        } else { // условие снижения

            //Условие установки самолета посередине экрана
            int setY = flight.getY();
            if (getY < screenY/2){
                setY = flight.getY() + (int)(30 * screenRatioY);
            } else if (getY > screenY/2) {
                setY = flight.getY() - (int)(30 * screenRatioY);
            }

            flight.setY(setY); // устанавливанет координаты
